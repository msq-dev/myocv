...
class SimpleThreshold(Trackbar):
    def __init__(self,
        ...
        super().__init__(name=f"{name} {threshold_type.capitalize()}",
                         init_val=252,
                         max_val=255)
    ...
    def process_images(self):
        _, self.simple_threshold = cv.threshold(self.blur,
                                                self.trackbar_value,
                                                self.max_val,
                                                self.type)
...
class CannyThreshold(Trackbar):
    ...
    def process_images(self):
        self.canny = cv.Canny(self.blur,
                              self.trackbar_value,
                              self.trackbar_value * 2)
...
class AdaptiveThreshold(Trackbar):
    def __init__(self,
        ...
        super().__init__(name=f"{name} {threshold_type.capitalize()} C",
                         init_val=3,
                         max_val=555)
    ...
    def process_images(self):
        ...
        self.adapt = cv.adaptiveThreshold(self.blur,
                                          255,
                                          self.type,
                                          cv.THRESH_BINARY,
                                          self.trackbar_value, 2)
...
class OtsuThreshold(Trackbar):
    ...
    def process_images(self):
        _, self.otsu = cv.threshold(self.blur,
                                    self.init_val,
                                    self.max_val,
